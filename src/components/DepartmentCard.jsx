...
export default function DepartmentCard({ department, index, isFilterActive }) {
        ...
        <div>
          <p className="font-mono text-[0.65rem] uppercase tracking-[0.2em] text-ink-faint">
            {department.isUnassigned
              ...
              : isFilterActive
              ? "Shortlisted"
              : `${department.questionCount} Questions`}
          </p>
          ...
          <div>
            <span
              className={`font-display text-4xl font-semibold ${
                isFilterActive && department.count > 0 ? "text-oxblood" : "text-ink"
              }`}
            >
              {department.count}
            </span>
            <span className="ml-2 font-mono text-xs uppercase tracking-wide text-ink-soft">
              {isFilterActive
                ? department.count === 1 ? "Candidate" : "Candidates"
